...
module.exports = {
    ...
    getProduct: (req, res) => Product.findOne({id: req.params.id})
    ...
    createProduct: (req, res) => Product.create(req.body)
                                        ...
                                        .catch(err => console.log("Backend returned err:", err)||res.json(err)), 
    updateProduct: (req, res) => Product.findOneAndUpdate({id: req.params.id}, req.body , {runValidators: true, new: true})
                                        .then(product => console.log("Backend selected product:", product)||res.json(product))
                                        .catch(err => console.log("Backend returned err:", err)||res.json(err)), 
    deleteProduct: (req, res) => Product.findOneAndRemove({id: req.params.id})
